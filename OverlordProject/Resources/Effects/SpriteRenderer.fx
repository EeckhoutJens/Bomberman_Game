...
void CreateVertex(inout TriangleStream<GS_DATA> triStream, float3 pos, float4 col, float2 texCoord, float rotation, float2 rotCosSin, float2 offset, float2 pivotOffset)
{
    if (rotation != 0)
    {
		...
		//Transform to origin
        float2 originPos = float2(pos.x, pos.y);

        originPos.x -= offset.x;
        originPos.y -= offset.y;
        originPos.x -= pivotOffset.x;
        originPos.y -= pivotOffset.y;

		//Rotate
        float oldPos = originPos.x;
...
void MainGS(point VS_DATA vertex[1], inout TriangleStream<GS_DATA> triStream)
{
    ...
    float xOffset = gTextureSize.x * scale.x;
    float yOffset = gTextureSize.y * scale.y;
	...
	//VERTEX 1 [LT]
    float2 pivotOffset = float2(pivot.x * xOffset, pivot.y * yOffset);
