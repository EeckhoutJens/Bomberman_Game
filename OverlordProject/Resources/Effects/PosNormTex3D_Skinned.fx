...
struct VS_INPUT{
    ...
    float4 boneIndice : BLENDINDICES;
...
VS_OUTPUT VS(VS_INPUT input){
    ...
    {
        float index = input.boneIndice[idx];
        if (index > -1)
        {
            transformedPosition += input.boneWeight[idx] * mul(originalPosition, gBones[index]);
            transformedNormal += input.boneWeight[idx] * mul(float4(input.normal,0), gBones[index]);
        }
