...
float InputManager::m_LeftThumbDeadzone = 15000;
float InputManager::m_RightThumbDeadzone = 15000;
...
bool InputManager::AddInputAction(InputAction action, bool ForceAdd)
{
...
DirectX::XMFLOAT2 InputManager::GetThumbstickPosition(bool leftThumbstick, GamepadIndex playerIndex)
{
	...
	if (leftThumbstick)
	{
		...
		if (pos.x > -m_LeftThumbDeadzone && pos.x < m_LeftThumbDeadzone)pos.x = 0;
		if (pos.y > -m_LeftThumbDeadzone && pos.y < m_LeftThumbDeadzone)pos.y = 0;
	}
	else
	{
		pos = DirectX::XMFLOAT2(m_CurrGamepadState[pi].Gamepad.sThumbRX, m_CurrGamepadState[pi].Gamepad.sThumbRY);

		if (pos.x > -m_RightThumbDeadzone && pos.x < m_RightThumbDeadzone)pos.x = 0;
		if (pos.y > -m_RightThumbDeadzone && pos.y <  m_RightThumbDeadzone)pos.y = 0;
	}
...
void InputManager::StopVibration(GamepadIndex playerIndex)
{
	XINPUT_VIBRATION vibration;

	ZeroMemory(&vibration, sizeof(XINPUT_VIBRATION));

	vibration.wLeftMotorSpeed = static_cast<WORD>(0 * 65535);
	vibration.wRightMotorSpeed = static_cast<WORD>(0 * 65535);
