...
void GameScene::RootUpdate()
{
	...
	std::sort(m_PostProcessingEffects.begin(), m_PostProcessingEffects.end(), [](PostProcessingMaterial* a, PostProcessingMaterial* b) {return a->GetRenderIndex() < b->GetRenderIndex(); });
...
void GameScene::RootDraw()
{
	...
	if (!m_PostProcessingEffects.empty())
	{
		...
		RenderTarget* previous_RT;
		previous_RT = init_RT;

		for (auto effect : m_PostProcessingEffects)
		{
			RenderTarget* temp_RT = effect->GetRenderTarget();
			SceneManager::GetInstance()->GetGame()->SetRenderTarget(temp_RT);
			effect->Draw(m_GameContext, previous_RT);
			previous_RT = temp_RT;
		}
		SceneManager::GetInstance()->GetGame()->SetRenderTarget(init_RT);
		SpriteRenderer::GetInstance()->DrawImmediate(m_GameContext, previous_RT->GetShaderResourceView(), DirectX::XMFLOAT2(0, 0));
