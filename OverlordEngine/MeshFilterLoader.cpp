...
MeshFilter* MeshFilterLoader::LoadContent(const std::wstring& assetFile)
{
	...
	for (;;)
	{
		...
		{
			pMesh->m_HasElement |= (UINT)ILSemantic::BLENDINDICES;

			//TODO: Start parsing the BlendIndices for every vertex
			//and add them to the corresponding vector
			//pMesh->m_BlendIndices
			for (int i{}; i < int(pMesh->m_VertexCount); ++i)
			{
				pMesh->m_BlendIndices.push_back(DirectX::XMFLOAT4(
					binReader->Read<float>(), binReader->Read<float>(), binReader->Read<float>(), binReader->Read<float>()));
			}
		}
		break;
		case MeshDataType::BLENDWEIGHTS:
		{
			pMesh->m_HasElement |= (UINT)ILSemantic::BLENDWEIGHTS;
			...
			for (int i{}; i < int(pMesh->m_VertexCount); ++i)
			{
				pMesh->m_BlendWeights.push_back(DirectX::XMFLOAT4(
					binReader->Read<float>(), binReader->Read<float>(), binReader->Read<float>(), binReader->Read<float>()));
			}
		}
		break;
		case MeshDataType::ANIMATIONCLIPS:
		{
			...
			for (int i{}; i < clipCount; ++i)
			{
				...
				const unsigned short keyCount{ binReader->Read<unsigned short>() };
				for (int j{}; j < keyCount; ++j)
				{
					AnimationKey key{};
					key.Tick = binReader->Read<float>();
					const unsigned short boneCount{ binReader->Read<unsigned short>() };
					for (int k{}; k < boneCount; ++k)
					{
